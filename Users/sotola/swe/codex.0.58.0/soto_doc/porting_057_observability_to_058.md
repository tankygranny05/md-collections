...
- The footer keys (`metadata`, `flow_id`, `round`, `data_count`, `sid`) must always appear in that order; `sid` is the final key.
...
- `round` is a top-level UUIDv7 string that rotates on every `turn.user_message` so downstream tooling can correlate envelopes; it must never disappear once logging starts.
- `pid` and `cwd` must live **inside** `metadata` (not as top-level keys).

### 1.1 `turn.exec.end` payload cap

- `turn.exec.end` envelopes must never exceed **300 KB**, even when other SSE lines keep the 5 MB allowance. Enforce this in two layers:
  - Pre-truncate the problematic fields before `serde_json::to_string`: `payload.stdout` and `payload.aggregated_output` each get a 128 KiB UTF‑8 aware budget, while `payload.formatted_output` gets a third of that (~42 KiB). Apply the standard `... [truncated after N bytes, omitted M bytes]` suffix so analysts can spot the clamp.
  - When writing to `sse_lines.jsonl`, force the logger to use `min(global_limit, 300 KB)` for `turn.exec.end` so escaped JSON never bloats past the per-event ceiling.
- Downstream documentation **must** mention these budgets anywhere the SSE size limits are discussed so investigations immediately understand why certain fields shrink at 128 KiB / 42 KiB boundaries.
