...
# Handover – Observability Port (0.60.1)

## TL;DR
- Reintroduced the full Sotola observability stack (CLI `--log-dir`, `[sotola.*]` config, centralized SSE/sessions/requests loggers, and turn-event plumbing) so 0.60.1 mirrors the same signal set as 0.58.0.
...
4. **Turn logging** – Ported `turn_logging.rs` and reconciled every `EventMsg` variant from 0.60.1’s protocol crate (new additions: `turn.mcp_startup.update` / `.complete`).
...
- **Mitigation:** Compared `codex-rs/protocol/src/protocol.rs` between 0.58.0 and 0.60.1, added handlers for new events, and re-ran the smoke harness each time to ensure the envelope order and queueing logic still behave. Future agents should repeat this reconciliation whenever protocol changes land.
